...
(function ( $, window, document, undefined ) {
	var pluginName = 'dpnAutocomplete',
			defaults = {
				...
				categories: [],
				minChars: 3,
				autocompleteId: 'dpn-autocomplete',
				autocompleteClasses: 'dpn-autocomplete',
	...
	$.extend(Plugin.prototype, {
		...
		initDropdown: function() {
			var html = '<div id="' + this.settings.autocompleteId + '" class="' + this.settings.autocompleteClasses + '" style="display: none;"><ul>';
			...
			this.$autocomplete = $('#' + this.settings.autocompleteId);
		},
		initEvents: function() {
			var that = this;
			...
			$list.on('click', 'li.item', function(event) {
				if (typeof that.settings.clickCallback === 'function') {
					that.settings.clickCallback(event, this);
				}
